...
public:
  ...
  virtual node_type __rtti_get_node_type() const {}

  // get_name will return the name of the type, NULL if no name (like builtin)
  virtual const char * __rtti_get_name() const {}

  // compare if type represented by the type_info are the same type
  virtual int __rtti_compare(const type_info&) const {}
...
public:
  enum builtin_type_val {
    _RTTI_BI_CHAR = 1, _RTTI_BI_SHORT, _RTTI_BI_INT, 
